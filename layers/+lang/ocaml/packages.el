...
(setq ocaml-packages
  '(
    ;; auto-complete
    company
    (flycheck :when (configuration-layer/layer-used-p 'syntax-checking))
    (flycheck-ocaml :when (configuration-layer/layer-used-p 'syntax-checking))
    ggtags
    counsel-gtags
    helm-gtags
    merlin
...
(defun ocaml/post-init-flycheck ()
  (when (configuration-layer/package-used-p 'flycheck-ocaml)
